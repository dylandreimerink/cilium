	...
	return Command(
		CmdUsage{
			...
			AutocompleteArgs: func(state *State, before []string, cur string) []string {
				...
				if cur == "" || strings.HasPrefix("stdout", cur) {
					suggestions = append([]string{"stdout"}, suggestions...)
				}
				if cur == "" || strings.HasPrefix("stderr", cur) {
					suggestions = append([]string{"stderr"}, suggestions...)
				}
