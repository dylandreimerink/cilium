...
func NewEndpointFromChangeModel(ctx context.Context, owner regeneration.Owner, policyGetter policyRepoGetter, namedPortsGetter namedPortsGetter, proxy EndpointProxy, allocator cache.IdentityAllocator, base *models.EndpointChangeRequest, legacyMetric *metrics.LegacyMetrics) (*Endpoint, error) {
	if base == nil {
		return nil, nil
	}

	ep := createEndpoint(owner, policyGetter, namedPortsGetter, proxy, allocator, legacyMetric, uint16(base.ID), base.InterfaceName)
