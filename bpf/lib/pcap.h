...
cilium_capture4_classify_wcard(struct __ctx_buff *ctx)
{
	...
	_Pragma("unroll") for (i = 0; i < size; i++)
	{
		cilium_capture4_masked_key(&okey, &prefix_masks[i], &lkey);
		match = bpf_map_lookup_elem(&CAPTURE4_RULES, &lkey);
		if (match)
			return match;
	}
...
cilium_capture6_classify_wcard(struct __ctx_buff *ctx)
{
	...
	_Pragma("unroll") for (i = 0; i < size; i++)
	{
		...
		match = bpf_map_lookup_elem(&CAPTURE6_RULES, &lkey);
		if (match)
			return match;
	}
						 ...
						 __u16 *cap_len __maybe_unused)
{
	if (capture_enabled)
	{
		...
		c = bpf_map_lookup_elem(&cilium_capture_cache, &zero);
